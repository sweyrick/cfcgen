	...
	&lt;cffunction name="set<xsl:value-of select="@name" />" access="public" returntype="void" output="false"&gt;
		...
		&lt;cfset variables.instance['<xsl:value-of select="@name" />'] = <xsl:choose><xsl:when test="@name='date_created'">valDate(trim(arguments.<xsl:value-of select="@name" />))</xsl:when><xsl:otherwise>trim(arguments.<xsl:value-of select="@name" />)</xsl:otherwise></xsl:choose> /&gt;
	&lt;/cffunction&gt;
